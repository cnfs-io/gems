...
module Pim
  module Commands
    class Serve < Dry::CLI::Command
      ...
      option :automation, type: :string, default: "preseed", values: %w[preseed kickstart],
             desc: "Serve a preseed (Debian) or kickstart (Fedora) config"
      ...
      def call(profile_name: nil, port: nil, preseed: nil, install: nil, automation: "preseed",
        ...
        kickstart = automation == 'kickstart'

        unless kickstart ? profile.kickstart_template(preseed) : profile.preseed_template(preseed)
          puts "Error: No #{automation} template found for '#{preseed_name}'"
          puts "Expected locations:"
          puts kickstart ? "  - ./resources/kickstarts/#{preseed_name}.ks.erb" : "  - ./resources/preseeds/#{preseed_name}.cfg.erb"
          Pim.exit!(1)
        end
